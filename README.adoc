...
----
/system script
  add dont-require-permissions=no policy=read,test \
    name=FreeDNS source="#!rsc\
    \n#\
    \n# Requires RouterOS\
    \n#\
    ...
    \n:local syncURL;                 # eg. \"http://sync.afraid.org/u/your_to\
    ken/\"\
    \n\
...
[IMPORTANT] Either way one has a specify a sync URL to be called within the script
